namespace GoogleSheetsHelper
{
    ...
    {
        public async Task ClearDataAsync(string range, CancellationToken ct = default)
        {
        ...
        public async Task<IList<IList<object>>> GetDataAsync(string range, CancellationToken ct = default)
        {
        ...
        public async Task<IList<IList<object>>> GetOrAddSheet(string range, CancellationToken ct = default)
        {
            try
            {
                return await GetDataAsync(range, ct);
            }
            catch (Exception ex)
            {
                if (ex.Message.Contains("Unable to parse range")) // не найдена таблица
                {
                    ...
                    return await GetDataAsync(range, ct).ConfigureAwait(false);
                }
                else
                {
                    throw;
